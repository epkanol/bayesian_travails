...
issue_model <- ulam(
  alist(
    y ~ dzipois(p, lambda),
    logit(p) <- ap + bpdup * DUP + bpadd * ADD, 
    log(lambda) <- alpha[author,team] + badd[author,team] * ADD, 
    ...
    transpars> matrix[author,11]:badd <- compose_noncentered(sigma_badd, L_Rho_badd, z_badd),
    matrix[11, author]:z_author ~ normal(0, 1),
    matrix[11, author]:z_badd ~ normal(0, 1),
    
    ap ~ dnorm(0, 1.5),
    bpdup ~ dnorm(0, 0.2),
    bpadd ~ dnorm(0, 0.2),
    vector[11]:sigma_author ~ dexp(1),
    vector[11]:sigma_badd ~ dexp(1),
    cholesky_factor_corr[11]:L_Rho_author ~ lkj_corr_cholesky(2),
    cholesky_factor_corr[11]:L_Rho_badd ~ lkj_corr_cholesky(2),
    # compute ordinary correlation matrixes from Cholesky factors
    gq> matrix[11,11]:Rho_author <<- Chol_to_Corr(L_Rho_author),
    gq> matrix[11,11]:Rho_badd <<- Chol_to_Corr(L_Rho_badd)
