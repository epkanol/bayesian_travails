...
We will get complaints from brms when we want to predict values using newdata, if we try to specify the value as a logical (either the constant F, or `as.logical(FALSE)` yields the same result):

We get `New factor levels are not allowed. Levels allowed: 'FALSE', 'TRUE'. Levels found: '0'`

```{r, eval=FALSE}
newdata <- data.frame(t(c(file=17, author=24, team=6, logDEL=log(100), DELFILE=F, DUP=10/scale_dupblocks$stddev)))
...
```{r}
data_factor <- data |> mutate(DELFILE_factor = as.factor(ifelse(DELFILE, 2, 1))) |> select(team, author, logDEL, DELFILE_factor, DUP, y)
M_delfile_as_factor <-
...
newdata <- data.frame(t(c(file=17, author=24, team=6, logDEL=log(100), DELFILE_factor=1, DUP=10/scale_dupblocks$stddev)))
