...
* My model does a good job in recovering the "scalar parameters", ap and al.
* Why does my model fail to recover "the real betas" (3 and -1 respectively, in my model)?
* How, in general, would a negative beta be recovered? (In my example, bscout is -1 in the generated data). Seeing that exp(bscout) is always positive?
...
The Normal(0, 0.4) prior allows some strong relationships, while keeping most of the lines relatively straight.
...
Most of the priors are relatively flat, but a few show strong coupling.